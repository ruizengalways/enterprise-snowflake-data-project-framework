...
def replace_first(path: str, old: str, new: str) -> None:
    ...
    if old not in text:
        raise SystemExit(f"expected patch marker not found in {path}: {old[:80]!r}")
    p.write_text(text.replace(old, new, 1), encoding="utf-8")


apply = "src/enterprise_snowflake_framework/pipeline_apply.py"
replay = "src/enterprise_snowflake_framework/pipeline_replay.py"
