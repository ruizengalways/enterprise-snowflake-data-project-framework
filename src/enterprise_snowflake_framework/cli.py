...
def _add_execution_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="Version-specific Silver execution technology; defaults to stream_task for standard patterns.",
    )
    parser.add_argument("--target-lag", help="Dynamic Table staleness target, for example '5 minutes'.")
    parser.add_argument(
        "--warehouse",
        help=(
            "Version execution warehouse. Used by a Stream/Task task or Dynamic Table refresh; "
            "defaults to the domain transform warehouse."
        ),
    )
    parser.add_argument(
        "--refresh-mode",
        choices=("incremental", "full"),
        help="Dynamic Table refresh mode. AUTO is deliberately not generated by the Framework.",
    )
    parser.add_argument(
        "--task-minimum-trigger-interval-seconds",
        type=int,
        help=(
            "Minimum interval for a stream-triggered task. Valid only for stream_task "
    ...
    parser.add_argument(
        ...
        type=int,
        help="Single task-run timeout in seconds; rendered as USER_TASK_TIMEOUT_MS.",
    )
    parser.add_argument(
        "--task-suspend-after-failures",
        type=int,
        help="Consecutive failed standalone task runs before Snowflake suspends the task.",
    )
    parser.add_argument(
        "--task-error-integration",
        help="Existing Snowflake notification integration for task error notifications.",
    )


def _task_args(args: argparse.Namespace) -> dict[str, object]:
    return {
        "task_minimum_trigger_interval_seconds": args.task_minimum_trigger_interval_seconds,
        "task_timeout_seconds": args.task_timeout_seconds,
        "task_suspend_after_failures": args.task_suspend_after_failures,
        "task_error_integration": args.task_error_integration,
    }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    scaffold_version_parser = subparsers.add_parser(
        "scaffold-version",
        help="Create a new candidate implementation and choose its execution model without modifying active code.",
    )
    ...
    validate = subparsers.add_parser("validate", help="Validate project, source, RAW, Silver and version contracts.")
...
def _print_plan(plan: SourcePlan) -> None:
    ...
    for item in plan.existing:
        ...
        if item.missing_standard_files:
            ...
            print(
                "    WARNING: existing dataset directory does not match its execution-specific scaffold layout; "
