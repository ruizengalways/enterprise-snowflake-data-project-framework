...
Execution mechanics remain separate. Explicit Stream/Task and batch implementations use `CONTROL.PIPELINE_RUN`; Dynamic Tables use native refresh history through `CONTROL.DYNAMIC_TABLE_REFRESH_STATUS_V`.
...
DQ and comparison evidence must be at least as new as the latest candidate runtime evidence. Candidate catch-up/refresh therefore happens before release preflight. A cutover must not create a new transformation run after accepted DQ/comparison evidence and then publish it without revalidation.
...
Rollback does not pretend that `RESUME` or `REFRESH` alone proves the previous implementation has caught up. Operators must establish target readiness before using the rollback operation when data has advanced since cutover.
